...
- **悬停**刘海展开列表（最多 6 行）。
- **点击任意一行**在桌面端打开那次对话，走 `claude://resume?session=<CLI id>`——
  这是外部唯一能真正聚焦会话的路由。桌面端没见过的会话会先被导入，再点一次就是
  聚焦上一次导入的那个，所以侧边栏每个会话最多多出一条。另外两条路由
  `code/continue` 和 `code/needs-input` 挂着账号级 gate，关着时什么都不做。

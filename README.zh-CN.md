...
一只吉祥物，七个表情，画在 15×12 的像素网格上。身体穿的是状态色——不用读任何图形，
一眼就知道**现在是什么情况**；轮廓和那只橙色的喙负责身份。眼神和右上角的像素徽标
说明具体是哪一种状态。

每个表情都有自己的一套动作。所有动作都发生在网格上——整格移动、逐帧停留、不做插值——
所以小鸡跳到半空时和站着时一样锐利；状态变化是*演出来*的，不是切过去的：小鸡先做一个
进场反应，身体的颜色再渐变到下一个颜色，而不是硬切。

| | 状态 | 徽标 | 含义 |
|:--:|:--|:--:|:--|
| <img src="docs/mascot/running.png" width="52"> | `running` 运行中 | 蓝色，无徽标 | 正在输出或跑工具。冷色后退——干着活恰恰是你最不用操心的事。2.6 秒一轮：起跳、屈膝落地、眨两下眼。 |
| <img src="docs/mascot/waiting.png" width="52"> | `waiting` 等待回复 | 橙色 **?** | 停在只有你能回答的地方：提问、计划确认、权限弹窗。品牌橙只花在这一种状态上。会连点两下头、张嘴叫你，徽标在背后一闪一闪。 |
| <img src="docs/mascot/stalled.png" width="52"> | `stalled` 阻塞 | 红色 **!** | 一个工具调用超过宽限期还没回来。脸颊上滑下一滴汗，然后整个人矮一格：是累，不是急——它等的是工具，不是你。 |
| <img src="docs/mascot/done.png" width="52"> | `done` 已完成 | 绿色 **✓** | 这一轮结束了，没有什么在烧。进场时蹦一下，之后就只是呼吸。 |
| <img src="docs/mascot/error.png" width="52"> | `error` 出错 | 红色 **✕** | 出问题了。横向抖一阵，然后不动。 |
| <img src="docs/mascot/idle.png" width="52"> | `idle` 空闲 | 灰色 **z** | 放着太久已经算杂音，收进底部一行。睡着了，角上飘着一个往上升的 z。 |
| <img src="docs/mascot/spent.png" width="52"> | `spent` 额度用尽 | 红色 **—** | 用量窗口跑空了，所有会话都停在同一件没有会话能解决的事上。没有新的颜料：灰色本来就是"什么都不动了"，红色本来就是"这事要紧"。唯一完全不动的表情——只有那根横杠在脉动，因为此刻还在走的只有一个时钟。 |
...
| 部件 | 说明 |
|:--|:--|
| Agent 标记 | 这是谁的会话：Claude Code 是那只终端小生物，Codex 是一个环。用形状区分，因为旁边的吉祥物已经把颜色花在状态上了。Claude 那个是它公开的标识一格不差搬过来的——那个标识本来就画在 24 单位方框里 1.5 单位的步长上，正好落在这个 app 的网格上，不用重画成"看起来像"。标记也会动：会话在跑的时候，Claude 那只迈步走、偶尔眨眼，Codex 的环上有一格亮点绕圈——停下来的会话就只是偶尔眨一下。行的左边和右边于是说的是同一件事。 |
...
拦下一次调用同时也就是把它拿走了：岛拿着的时候，agent 自己那个提示还没出现，
所以能回答的地方只有一个，而且未必是你正在看的那个。**点那一行就把它还回去**——
发的是超时用的同一个 `ask`——然后跳到那个会话，它此刻正按原本的方式在问你。
Roost 是替那个提示站一会儿，不是把它据为己有。
...
| `UserPromptSubmit`、`Stop` | 一轮什么时候开始、什么时候结束——转录记得最晚也最差的两件事。两个都不等回答，只是把已经在屏幕上的那一行修正得更准。 |
...
没有常驻守护进程，不需要账号，不上报任何东西。Roost 只读你本来就有的文件；其中有些写得太晚、读了也没用，那部分改成被告知：
...
转录只记录一轮什么时候*结束*，从不记录什么时候开始，所以"人按了回车"这件事得从里面读出来：
一条 `user` 记录只要装的不是工具结果，就是新的一轮，它会作废前面那个 stop reason，
连同用户敲字时还挂着没回来的调用一起清掉。没有这一条，会话从你发出提示的那一刻起
就会一直报 `done`，直到模型的第一条消息落盘为止——实测中位数 **13 秒**，最长能到三分钟——
而这段空档恰恰就是这个 app 存在的意义。

### 从"读"改成"被告知"

上面那条修好了一轮的**开头**——提示词发出后 0.2 秒就落盘了。但它修不了**结尾**。
一轮的最后一条消息要等模型把答复写完才会落盘——按这台机器上的会话实测，
中位数 **15 秒**——而这 15 秒里答复早就显示在你屏幕上了，行里却还是"忙"。

光靠读解决不了，所以 Roost 不再只是读。装上 hook 之后，Claude Code 会额外播报
`UserPromptSubmit` 和 `Stop` 两个事件，它们就在边界那一刻触发。文件继续负责说
每个会话**是什么**——项目、标题、模型、工具和它的参数——hook 只决定**这一轮什么时候翻页**。
谁说得晚谁算数：`Stop` 盖过还没跟上的转录，而下一行写进转录的内容又会盖过 `Stop`。

Claude Code 的会话永远不会像 Codex 那样*由* hook 生出一行。它有自己的注册表，
凭 hook 再生一行就是重复，而且会比进程本身多活十二个小时。
...
它默认是关的，第一次读令牌会弹一次钥匙串授权，更新之后会再弹一次——ad-hoc 签名让每个构建
都是一个新身份，所以「始终允许」的有效期恰好等于那个构建的寿命。令牌**只读一次**，
然后一直留着直到它自己过期，而不是每次轮询都读：按一分钟一次的轮询，重复读会把一次授权
变成一分钟一个弹窗。什么都没读到时，也只是每十五分钟再看一眼，而不是每一拍都看。
...
额度用尽的窗口不等计时器，直接走宽的那一档：它是唯一右侧放的是一个钟而不是一个数字的状态，
而 `4时48分` 在常规宽度里放不下——英文勉强，中文差得更多。

数字本身只在有会话阻塞时才穿那个实心圆：那时它是同一档升级、同一个颜色，
而深色压在亮色上正是让一个数字在几米外还看得清的原因。安静的状态就只有数字本身——
给一个没人需要对它做什么的数字画一圈，只是在跟真正该吸引视线的那个标记抢分量。

...
- **悬停**刘海展开列表（最多 6 行）。
- **点击任意一行**打开那次对话，走 `claude://resume?session=<CLI id>`——外部唯一
  能真正聚焦会话的路由。它只在**不会留下副本**时才发：桌面端从没见过的会话（导入
  正是目的），以及它已经导入过的会话（这次是找到而不是新建）。
- 桌面端自己创建的会话是例外。它给这类会话分配了自己的 id，而导入只按
  `local_<CLI id>` 去重，所以导入会落在原会话**旁边**而不是它身上。另外两条吃桌面
  id 的路由 `code/continue` 和 `code/needs-input` 挂着账号级 gate，什么都不做。
  这类点击只会把应用拉到前面——这个答案不好，但比多出一份对话副本要好。

...
| <img src="docs/mascot/spent.png" width="52"> | `spent` 额度用尽 | 红色 **—** | 用量窗口跑空了，所有会话都停在同一件没有会话能解决的事上。没有新的颜料：灰色本来就是"什么都不动了"，红色本来就是"这事要紧"。 |
...
| 部件 | 说明 |
|:--|:--|
| Agent 标记 | 这是谁的会话：Claude Code 是那只终端小生物，Codex 是一个环。用形状区分，因为旁边的吉祥物已经把颜色花在状态上了。Claude 那个是它公开的标识一格不差搬过来的——那个标识本来就画在 24 单位方框里 1.5 单位的步长上，正好落在这个 app 的网格上，不用重画成"看起来像"。 |
...
两个开关，因为通往同样两个数字的路有两条，而它们的代价完全不同。

本机上这两个数字只出现在 status line 里——转录里没有，统计缓存里也没有——
所以*从 status line 读用量*会装一个 status line 命令，它**包住**你原本配置的那个：
同一份载荷送进它的 stdin，它的输出原样打出来，而 Roost 写进去的那条记录
把原来的整条用 base64 带在自己的参数里。再关掉就原样还回去，只靠设置文件本身，
Roost 开着没开着都一样。它做不到的是：没有人在渲染 status line 的时候就没有数字——
桌面 App 不渲染，两次会话之间的 Mac 也不渲染。

*向 Anthropic 查询剩余用量*补的就是这一段，也是除了更新检查之外 Roost 唯一往外发的东西。
大约每分钟向 `api.anthropic.com` 问一次你自己账号的用量，用的是 Claude Code
已经存在钥匙串里的那个令牌。令牌只读、不写回：那个钥匙串条目归 Claude Code 所有，
由它按自己的节奏轮换，两个进程抢同一条记录换来的是登录被弄坏——为了底部的一个数字，
这个价太贵。令牌过期就当作没有凭据，status line 那份读数继续顶着，
等 Claude Code 下次轮换。会话的任何信息都不会跟着请求出去。
它默认是关的，第一次读令牌会弹一次钥匙串授权，更新之后会再弹一次。

两个窗口在底部各占一条十格的轨道，画在小鸡所用的同一张像素网格上：
要回答的问题是"还剩几格"，而格子可以数，进度条不行。
鼠标停上去，重置时刻会摊开在整条底栏上，后面跟着分模型的周用量。

一段时间没人刷新的读数会变暗并标上时间，而不是直接消失。
有人打开岛就是想问还剩多少，给他"什么都没有"比给他"一个数字和它成立的时刻"更差；
而重置时刻已经过去的窗口会直接说"已重置"，不会摆一个早就不成立的数字。

用掉八成以上时，这个窗口会从 header 的 headline 里把"几个在跑"顶掉，
因为它才是会让所有东西停下来的那件事——而且它说的是什么时候恢复，不是用了多少。
真的用尽、机器上又还有活的时候，收起态的岛会变形，小鸡换上上面那张脸，
倒计时站在原本放计数的位置。

除了这个轮询，Roost 发出的唯一请求是每 6 小时向 GitHub 的 releases API
查一次有没有新版本，除了请求本身不带任何信息。有新版时齿轮上出现一个点、
底部多一行提示；安装仍然是手动的——ad-hoc 签名的包没有值得校验的签名，
不该自己覆盖自己。不想要的话在菜单里关掉 *Check automatically*。
...
要不要连 Codex 一起看着、状态变化要不要出声、以及用量窗口走两条路里的哪一条。**退出也在这里**——accessory 应用没有 Dock 图标可以退。
...
[Apache License 2.0](LICENSE)。可以自由使用、修改、分发——个人或公司，开源项目
或闭源产品都可以。它要求的只是保留声明、附上许可证副本，并说明你改了什么。

v0.2.0 及以前的版本以 MIT 发布，那些版本仍然是 MIT。
